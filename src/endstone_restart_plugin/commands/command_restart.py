...
class CommadRestart(CommandExecutor):
    ...
    def vote(self, sender: Player, vote_type: str):
        if int(self.restart_data["vote"]["start"]) == 1:
            ...
            if vote_type == "no":
                ...
                no_vote_type = "no"  
            try:self.restart_data["vote"]["data"][no_vote_type].remove(sender.xuid)
            except:pass
            
            if sender.xuid in self.restart_data["vote"]["data"][vote_type]:
                return
            
            self.restart_data["vote"]["data"][vote_type].append(sender.xuid)
